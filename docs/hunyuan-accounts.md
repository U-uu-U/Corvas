...
- 发送成功、进入 Rhino 整理后，两处临时提示卡自动撤掉，完成时不再弹出一张常驻提示；后续进度保留在 Agent 任务卡中。混元页常驻导入入口保留，处理中显示「已提交」。需要确认、连接处理或错误核对的提示仍会显示。
- 切换成手动会拦住尚未开始导入、且没有明确确认的任务；已经开始的 Rhino 操作继续执行。多个模型依次处理。
- Rhino 或 Grasshopper 因启动提示延迟时，任务保持「等待 Rhino / Cordyceps 连接」；关闭提示后自动接续，也可点击「重试连接」。已完成的 FBX 下载会复用，即使混元窗口随后关闭也无需重新下载。旧版本中仅因 Rhino 启动超时失败、且还未开始导入的任务会恢复为等待状态。
