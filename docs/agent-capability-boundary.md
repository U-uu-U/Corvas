...
状态：已按当前 Codex 本机对话入口实施。工作台交接、独立 MCP 调用和内部 Agent 的能力边界已落地；不包含 ChatGPT 网页端远程连接。可执行契约见 [MCP 外部软件连接](mcp-client.md)。
...
本次复用 Corvas 已配置的软件 MCP 连接，由 `handoff.tools/call` 向外部助手提供真实 schema 和独立调用入口。外部助手负责推理，Corvas 负责连接、检查点和回执，不启动内部文字模型。也可在外部助手中直接连接软件 MCP，但应自行协调同一场景的任务状态，避免并行修改。

## 连接方式

Corvas 对外接口是 `mcp/flow-canvas-mcp.mjs` 的本机 stdio 包装器，转发到仅接受 localhost 的桥接服务。现已提供画布、固定 `workflow.*` 和外部助手 `handoff.*`；软件工具通过绑定项目和任务的接口调用。不提供远程 MCP 认证或自动向 ChatGPT/Codex 发消息的接口。

- 当前 Codex 对话：使用本机 MCP；工作台创建持久交接并提供复制指令，Codex 主动读取/接手任务。仅连接 MCP 不会自动唤醒对话。
...
原实现通过 `AgentRuntime.start({ execution: 'rhino_cleanup' })` 启动整理。本次改为受信任的 `startRhinoCleanup()`，实际任务、项目和导入报告均需校验；继续复用已有任务记录载体，避免破坏历史检查点。

本次兼容处理：

1. 普通 Agent 启动入口与受信任工作流入口分开。普通 IPC/MCP 请求不能通过 `execution` 或伪造工作流来源获得外部操作能力；固定工作流入口校验任务和项目绑定，只运行限定的整理阶段。
2. 固定执行与自动/手动设置独立于普通对话，保留 `workflow.*` 名称、工作流版本、任务 ID、历史记录和检查点。已有成功阶段复用，结果不明时查询状态，不能自动重放。历史运行记录的存储格式继续兼容。

历史普通 Agent 的外部任务保留可读记录。确认、重试或恢复旧外部调用时明确拒绝，并提示由外部助手接续；不会因为升级就执行或重新提交，也不会擅自把旧操作变成新的交接任务。既有画布创作 Skill 不需要清空。
