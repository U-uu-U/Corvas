...
powershell -NoProfile -Command "$manifest=@{name='com.flow.canvas_sync';description='Corvas browser task sync and download archive host';path='%HOST_RUNNER%';type='stdio';allowed_origins=@('chrome-extension://%EXT_ID%/')} | ConvertTo-Json -Depth 4; [IO.File]::WriteAllText('%HOST_MANIFEST%', $manifest, [Text.UTF8Encoding]::new($false))"
if errorlevel 1 (
