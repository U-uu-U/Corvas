...
import { isCatalogManaged, findCatalogEntry } from './model-catalog.mjs';
const PAUSED_MODELS = {
...
export function isVideoGenerationAvailable(provider, config) {
    if (isCatalogManaged(config, provider)) {
        const entry = findCatalogEntry(config, provider);
        return Boolean(entry && entry.catalog.enabled !== false);
    }
    let host;
...
export function assertVideoGenerationAvailable(provider, config) {
    if (!isVideoGenerationAvailable(provider, config)) {
