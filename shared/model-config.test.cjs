...
test('CSV 每一行都被默认 CONFIG 恰好一个条目覆盖，且 notes 逐字一致', async () => {
    ...
    assert.equal(rows.length, 22, 'CSV 行数变化时必须同步确认 CONFIG');
...
test('默认 CONFIG 的 refreshIntervalMs 是 1 小时，且不依赖远端即可给出 22 个模型', () => {
    const config = readConfig();
    assert.equal(config.refreshIntervalMs, 60 * 60 * 1000);
    assert.equal(config.models.length, 22);
    const kinds = config.models.reduce((acc, entry) => {
        acc[entry.kind] = (acc[entry.kind] || 0) + 1;
        return acc;
    }, {});
    assert.deepEqual(kinds, { image: 3, video: 17, text: 2 });
});
