...
const LOCAL_MESSAGE_PATTERNS = [
    ...
    /^(?:(?:Seedance(?: 2\.[05])?(?: Pro)?|HM-Seedance[\w. -]*|MiniMax H3|StarFrame|GlobalAiOpc|ch(?:0107|1401)-sd-2\.5-720p) )?(?:最多支持 \d+ (?:张参考图片|个参考视频|段参考音频)|(?:Pro )?仅支持 (?:480p 或 720p|720p)|(?:视频)?时长(?:仅支持|必须为|必须在) \d+ (?:到 \d+ 秒的整数|到 \d+ 秒之间)|不支持画幅比例 (?:\d+:\d+|adaptive))$/,
